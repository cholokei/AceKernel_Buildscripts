...
export KERNEL_NAME="cm-10.1-kernel"
...
then
	...
	#KERNEL_CMDLINE="androidboot.hardware=qcom msm_watchdog.appsbark=0 msm_watchdog.enable=1 loglevel=4"
...
if [ "$DEVICE" == "dalikt" ]
then
	./mkbootimg --kernel zImage --ramdisk ramdisk.gz --cmdline "androidboot.hardware=qcom msm_watchdog.appsbark=0 msm_watchdog.enable=1 loglevel=4" --base 0x48000000 --pagesize 2048 --ramdiskaddr 0x49400000 -o boot.img
#elif [ "$DEVICE" == "dalilgt" ]
#then
#
#elif [ "$DEVICE" == "daliskt" ]
