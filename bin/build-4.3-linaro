...
export PATH=$HOME/linaro-toolchain/4.8/android-toolchain-eabi/bin:$PATH
export ARCH=arm
export SUBARCH=arm
export CROSS_COMPILE=arm-eabi-
...
export READY_KERNEL="CM_LINARO_KERNEL"
export MODULES=`find -name *.ko`

export KERNEL_NAME="cm-10.2-linaro-kernel"
...
then
	...
	#KERNEL_CMDLINE="androidboot.hardware=qcom usb_id_pin_rework=true zcache"
...
if [ "$DEVICE" == "dalikt" ]
then
	./mkbootimg --kernel zImage --ramdisk ramdisk.gz --cmdline "androidboot.hardware=qcom usb_id_pin_rework=true zcache" --base 0x48000000 --pagesize 2048 --ramdiskaddr 0x49400000 -o boot.img
#elif [ "$DEVICE" == "dalilgt" ]
#then
#
#elif [ "$DEVICE" == "daliskt" ]
